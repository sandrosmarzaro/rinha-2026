...
| 10 | **~3496 sim** (3435-3580 banda) | Single global IVF + drop bbox sweep + Python partition_key (cleanup) | **+155 sim** | Replaced 84 per-partition IndexIVFScalarQuantizer com 1 global IndexIVFScalarQuantizer (nlist=2048, nprobe=12). Drop bbox cross-partition + extras loop por completo. Detection 2043→~2240 (FP -16, FN -3-9). **K-means coarse quantizer agrupa por L2 real** vs hand-crafted partition_key que era arbitrária — não precisa mais do bbox sweep pra compensar desalinhamento. 1 faiss.search/query (vs 1-9 antes). **Profile revelou que o gargalo k6 era throttle externo, não trabalho interno** — isso liberou CPU budget pra recall melhor: faiss_search interno subiu 469→963µs avg, MAS k6 score subiu +155 porque throttle dominou. Diff -89 LOC (removeu bbox, fallback partition índices, extras_nprobe, MAX_EXTRA_PARTITIONS). **Falta validar em prévia real** — sim sempre teve viés vs Haswell |
