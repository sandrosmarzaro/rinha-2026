...
/// Scan a single cluster, updating top5.
#[inline(always)]
...
/// Two-stage kernel with class-aware early-stop.
///
/// `cells_sorted` is expected to be ordered by ascending centroid distance.
/// Inner pass scans the first `inner_nprobe` cells; if the top-5 is unanimous
/// (fc == 0 or fc == 5) we exit immediately. Otherwise, if the verdict is
/// borderline (fc ∈ {2, 3}) we extend the scan to the remaining cells.
#[pyfunction]
fn knn_top5_smart(
    py: Python<'_>,
    query: PyReadonlyArray1<i16>,
    ...
    cluster_labels: PyReadonlyArray1<u8>,
    cluster_offsets: PyReadonlyArray1<i64>,
    cells_sorted: PyReadonlyArray1<i64>,
    inner_nprobe: i64,
) -> PyResult<i32> {
    let q = query.as_slice()?;
    let v = vectors.as_slice()?;
    let labels = cluster_labels.as_slice()?;
    let offsets = cluster_offsets.as_slice()?;
    let cells = cells_sorted.as_slice()?;
    let inner = (inner_nprobe as usize).min(cells.len());
    ...
    py.allow_threads(|| {
        ...
        // Phase 1: scan the inner_nprobe closest cells.
        let mut idx = 0usize;
        while idx < inner {
            let cell = cells[idx] as usize;
            let s = offsets[cell] as usize;
            let e = offsets[cell + 1] as usize;
            if use_avx2 {
        ...
        let fc = top5.fraud_count();
        if !(fc >= 2 && fc <= 3) {
            return;
        }

        // Phase 2: borderline → extend the scan to the remaining (outer) cells.
        while idx < cells.len() {
            let cell = cells[idx] as usize;
            let s = offsets[cell] as usize;
            let e = offsets[cell + 1] as usize;
...
fn knn_simd(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(knn_top5_smart, m)?)?;
    Ok(())
}
